...
m = 1470
c = 1005.4
M = 3600/20 # modified for hourly control
R = 4.329 * 10^-5 # modified for hourly control
h.ref <- 18
h0 <- 16
dt <- 1/2
Tf <- 24
...
for (i in 1:length(stati)){
  if (done[i]==0){
    azioni_ottime[i]<-optimal_action(stati_nuovo[i])
...
optimal_action_cal <- function(state) {
  q_vals <- predict(q_mod, data.frame(stato = state, azione = azioni_possibili))
...
for (i in 1:length(cal$stato)){
  if (cal$done[i]==0){
    azioni_cal[i]<-optimal_action_cal(stati_cal[i])[2]
    q_val_cal[i]<-optimal_action_cal(stati_cal[i])[1]
...
#   if (test$done[i]==0){
#     azioni_test[i]<-optimal_action_cal(stati_test[i])[2]
#     q_val_test[i]<-optimal_action_cal(stati_test[i])[1]
#     dQg = M * c * (azioni_test[i]-stati_test[i])     # gain
#     dQl = (stati_test[i]-test$CSIs[i]) / R   # loss
#     stati_test[i+1] = stati_test[i] + (dt/(m*c)) * (dQg-dQl)}
#   else {
#     azioni_test[i]<-optimal_action_cal(stati_test[i])[2]
#     q_val_test[i]<-optimal_action_cal(stati_test[i])[1]
